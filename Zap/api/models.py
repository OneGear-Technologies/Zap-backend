...
def generate_unique_code():
    ...
    while True:
        ...
        if User.objects.filter(code=code).count() == 0 and Stat.objects.filter(code=code).count() == 0:
            break
...
class Stat(models.Model):
    ...
    lock_stat = models.BooleanField(null=False, default=False)#status if lock is engaged or disengaged
